...
export default function AdminTicketDashboard() {
    ...
    const renderDetailView = () => {
        if (!selectedTicket) {
            return (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 border border-dashed border-slate-200 rounded-md">
        ...
        return (
            <div className="bg-white border border-slate-200 rounded-md flex flex-col h-full overflow-hidden shadow-sm">
                {/* Header */}
                    ...
                    <div className="flex gap-3 items-center">
                        {selectedTicket.status !== 'Closed' && (
                            <button
                                ...
                                className="flex items-center gap-1.5 bg-slate-50 text-slate-600 border border-slate-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200 px-3 py-1.5 rounded-md text-[12px] font-bold transition-colors"
                            >
                ...
                <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-6 bg-white">
                    ...
                    <div className="flex flex-col items-start">
                        <div className="max-w-[85%] p-4 text-[13px] shadow-sm bg-white border border-slate-200 text-slate-700 rounded-xl rounded-tl-sm relative">
                            ...
                            {mainAttachmentUrl && (
                                <div className="mt-4 pt-3 border-t border-slate-100">
                                    <div className="flex flex-wrap items-center gap-2 mb-3">
                                        ...
                                        <a href={mainAttachmentUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-md text-[11px] font-bold transition-colors border border-slate-200">
                                            <ExternalLink size={12} /> Open
                                        </a>
                                        <a href={mainAttachmentUrl} download="Attachment" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 rounded-md text-[11px] font-bold transition-colors border border-blue-100">
                                            <Download size={12} /> Download
                                        </a>
                                    </div>
                                    {!imgError && (
                                        <div className="block bg-slate-50 p-2 rounded-md border border-slate-100 inline-block">
                                            <img src={mainAttachmentUrl} alt="Ticket Preview" className="max-w-full sm:max-w-[280px] max-h-[180px] object-contain rounded-md" onError={() => setImgError(true)} />
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {selectedTicket.replies.map((reply: any, idx: number) => {
                                ...
                                return (
                                    <div key={idx} className={`flex flex-col ${reply.sender === 'Admin' ? 'items-end' : 'items-start'}`}>
                                        <div className={`max-w-[85%] p-4 text-[13px] shadow-sm ${reply.sender === 'Admin'
                                                ? 'bg-[#00b4d8] text-white rounded-xl rounded-tr-sm'
                                                : 'bg-white border border-slate-200 text-slate-700 rounded-xl rounded-tl-sm'
                                            }`}>
                                            <span className={`text-[10px] font-bold block mb-1.5 uppercase tracking-wider ${reply.sender === 'Admin' ? 'text-cyan-100' : 'text-slate-400'}`}>
                                            ...
                                            {replyAttachmentUrl && (
                                                <div className={`mt-3 pt-3 border-t ${reply.sender === 'Admin' ? 'border-cyan-400' : 'border-slate-100'}`}>
                                                    <a
                                                        href={replyAttachmentUrl}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className={`flex items-center gap-1.5 text-[11px] font-bold hover:underline mb-2 ${reply.sender === 'Admin' ? 'text-white' : 'text-[#00b4d8]'}`}
                                                    >
                                                    ...
                                                    <img
                                                        ...
                                                        className={`max-w-full sm:max-w-[200px] max-h-[120px] object-cover rounded-md border shadow-sm ${reply.sender === 'Admin' ? 'border-cyan-400' : 'border-slate-200'}`}
                ...
                {selectedTicket.status !== 'Closed' ? (
                    <div className="p-5 border-t border-slate-100 bg-white flex-shrink-0">
                        <textarea
                            ...
                            className="w-full bg-slate-50 border border-slate-200 p-3.5 text-[13px] text-slate-700 rounded-md outline-none focus:border-[#00b4d8] focus:ring-1 focus:ring-cyan-100 min-h-[90px] resize-none mb-3 transition-all placeholder:text-slate-400"
                        ></textarea>
                        <div className="flex justify-end">
                            <button
                                onClick={handleAdminReply}
                                disabled={isReplying || !adminReply.trim()}
                                className="bg-[#00b4d8] hover:bg-[#0096b4] text-white px-6 py-2.5 rounded-md text-[13px] font-semibold transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
                            >
                    ...
                    <div className="p-4 bg-slate-50 text-center text-[12px] font-bold text-slate-400 border-t border-slate-100 flex-shrink-0 rounded-b-md flex items-center justify-center gap-2">
    ...
    // ==========================================
    return (
        <div className="p-4 md:p-6 max-w-[1400px] mx-auto bg-slate-50 min-h-screen font-sans w-full flex flex-col">

            {/* Header Area (MATCHED WITH THEME) */}
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-3 pb-4 flex-shrink-0">
                <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-md text-[13px] text-slate-700 outline-none focus:border-[#00b4d8] transition-all shadow-sm h-9 placeholder:text-slate-400"
                        />
                    </div>
                    <select
                        value={statusFilter}
                        onChange={(e) => setStatusFilter(e.target.value)}
                        className="py-2 px-3 bg-white border border-slate-200 rounded-md text-[13px] text-slate-700 outline-none focus:border-[#00b4d8] cursor-pointer transition-all font-medium shadow-sm h-9"
                    >
            ...
            {/* Main Content Area */}
            <div className="flex-1 flex flex-col md:flex-row gap-6 overflow-hidden h-[calc(100vh-140px)] min-h-[600px]">

                {/* LEFT PANEL: Ticket List */}
                <div className={`md:w-1/3 flex flex-col bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden ${selectedTicket ? 'hidden md:flex' : 'flex'}`}>
                    <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center flex-shrink-0">
                        <h2 className="text-[14px] font-medium text-slate-800">Inbox <span className="text-slate-400 ml-1">({filteredTickets.length})</span></h2>
                        <button onClick={fetchAllTickets} className="text-[12px] text-[#00b4d8] hover:text-[#0096b4] font-semibold flex items-center gap-1.5 transition-colors">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 bg-slate-50/50">
                        {isLoading ? (
                            <div className="flex justify-center py-10"><Loader2 className="animate-spin text-[#00b4d8]" /></div>
                            ...
                            <div className="space-y-1.5">
                                {filteredTickets.map(ticket => (
                                    <div
                                        ...
                                        className={`p-4 rounded-md cursor-pointer transition-all border ${selectedTicket?.id === ticket.id
                                                ? 'bg-cyan-50/50 border-cyan-200 shadow-sm'
                                                : 'bg-white border-slate-200 hover:border-[#00b4d8]/50 shadow-sm'
                                            }`}
                                    >
                                        <div className="flex justify-between items-start mb-1.5">
                                            <span className="text-[13px] font-bold text-[#00b4d8]">{ticket.ticketId}</span>
                                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-[4px] uppercase border ${getStatusBadge(ticket.status)}`}>
                                                {ticket.status}
                                            </span>
                                        </div>
                                        <div className="text-[13px] text-slate-700 font-medium truncate mb-2">{ticket.subject}</div>
